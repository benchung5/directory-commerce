...
$country_name = dc_country_code_to_name( $country_code );
//if the query doesn't have a valid country, redirect.
    if ($country_name == null) 
            {
                echo "Sorry, we can't find the country you're looking for " . '<a href="lawworldwide.com/law-firm-listings/">Return to listings</a>' ;
            }
    else {
...
<div class="firms-page-header">
    <h1 class="firms-page-title"><?php echo stripslashes($country_name) ?></h1>
    ...
    if ($query->have_posts()) { ?>
        ...
        <div id="post-<?php echo the_ID(); ?>" <?php post_class(); ?> > 
            <div class="listing-entry">
                ...
                <div class="x-small-text"><span>Website: </span>
                <?php if(get_post_meta($post->ID, 'dc_firm_website', true)) { ?>
                        <a href="<?php echo get_post_meta($post->ID, 'dc_firm_website', true); ?>"><?php echo dc_strip_http(get_post_meta($post->ID, 'dc_firm_website', true)) ; ?> </a>
                   <?php } else { // elseif no website entered?>
                        <a href="#"><?php echo dc_strip_http(get_post_meta($post->ID, 'dc_firm_website', true)) ; ?></a>
                <?php } ?>
                <span>Address: </span><?php echo get_post_meta($post->ID, 'dc_address', true); ?> | <?php echo get_post_meta($post->ID, 'dc_city', true); ?>, <?php echo get_post_meta($post->ID, 'dc_state', true); ?>, <?php echo get_post_meta($post->ID, 'dc_city', true); ?></span></div>
