...
//city from slug
$city_name = $_GET['city'];
//if the query doesn't have a valid city, redirect.
    if ($city_name == null) 
            {
                echo "Sorry, we can't find the city you're looking for " . '<a href="lawworldwide.com/law-firm-listings/">Return to listings</a>' ;
            }
    else {
...
<div class="firms-page-header">
    <h1 class="firms-page-title"><?php echo stripslashes($city_name) ?></h1>
        ...
        <div class="no-break" id="post-<?php echo the_ID(); ?>" <?php post_class(); ?> > 
            <div class="listing-entry">
                ...
                <?php } ?>
                    ...
                    <br/><span class="small-text"><?php echo get_post_meta($post->ID, 'dc_address', true); ?> | <?php echo get_post_meta($post->ID, 'dc_city', true); ?>, <?php echo get_post_meta($post->ID, 'dc_state', true); ?>, <?php echo get_post_meta($post->ID, 'dc_city', true); ?></span></span>
            </div><!-- .entry-content -->
        </div><!-- #post -->
<?php 
endwhile;
